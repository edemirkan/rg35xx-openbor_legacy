...
#elif defined(OPENDINGUX) || defined(RG35XX)
...
int video_set_mode(s_videomodes videomodes)
{
	...
	bytes_per_pixel = videomodes.pixel = 2; // fix for miyoo
...
int video_copy_screen(s_screen* src)
{
    ...
    int x, y;
		...
		for(y=0; y<240; y++){
			for(x=0; x<320; x++){
				col = *s++;
				*d++ = SDL_MapRGB(ds->format, (col & 0x0000ff), (col & 0x00ff00) >> 8, (col & 0xff0000) >> 16);
			}
		}
	}
