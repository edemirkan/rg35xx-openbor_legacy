...
BUILDING        = 1
CC              = arm-linux-gcc
INCLUDES        = /opt/miyoo/arm-miyoo-linux-uclibcgnueabi/sysroot/usr/include/SDL
LIBRARIES       = 
...
ifdef BUILD_OPENDINGUX
STRIP           = arm-linux-strip $(TARGET) -o $(TARGET_FINAL)
endif
ifdef BUILD_RG35XX
STRIP           = arm-linux-strip $(TARGET) -o $(TARGET_FINAL)
endif
...
ifdef BUILD_RG35XX
INCS           += '/opt/miyoo/arm-miyoo-linux-uclibcgnueabi/sysroot/usr/bin/libpng-config --prefix'/include/libpng16
endif
...
CFLAGS 	       += -D_REENTRANT -DOPENDINGUX -DOPENDINGUX_CONTROL -DGCW0 -Os -marm -mtune=cortex-a9 -mfpu=neon-fp16 -mfloat-abi=softfp -march=armv7-a
else
CFLAGS 	       += -D_REENTRANT -DOPENDINGUX -O2 -fomit-frame-pointer -ffunction-sections -ffast-math -fsingle-precision-constant -march=armv5te -mtune=arm926ej-s
endif
endif
...
CFLAGS 	       += -DRG35XX_BUTTON_RIGHT=SDLK_d
CFLAGS 	       += -DRG35XX_BUTTON_A=SDLK_a
CFLAGS 	       += -DRG35XX_BUTTON_B=SDLK_b
CFLAGS 	       += -DRG35XX_BUTTON_X=SDLK_x
CFLAGS 	       += -DRG35XX_BUTTON_Y=SDLK_y
CFLAGS 	       += -DRG35XX_BUTTON_L1=SDLK_h
CFLAGS 	       += -DRG35XX_BUTTON_L2=SDLK_j
CFLAGS 	       += -DRG35XX_BUTTON_R1=SDLK_l
CFLAGS 	       += -DRG35XX_BUTTON_R2=SDLK_k
CFLAGS 	       += -DRG35XX_BUTTON_START=SDLK_m
CFLAGS 	       += -DRG35XX_BUTTON_SELECT=SDLK_n
CFLAGS 	       += -DRG35XX_BUTTON_MENU=SDLK_u
endif
